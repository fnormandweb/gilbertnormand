...
function render() {
  ...
  const slides = Array.isArray(t.heroSlides) && t.heroSlides.length > 0 ? t.heroSlides : [
  ...
  const heroSlidesHTML = slides.map((s, i) => `<div class="hero-slide ${i === 0 ? 'active' : ''}" data-slide="${i}" aria-current="${i === 0 ? 'true' : 'false'}" aria-hidden="${i !== 0}"><img class="hero-bg" src="${s.image}" alt="${(s.alt || '').replace(/"/g, '&quot;')}" /><div class="hero-smoke-overlay"></div></div>`).join('')
  const heroIndicatorsHTML = slides.map((s, i) => `<button type="button" class="hero-slider-indicator ${i === 0 ? 'active' : ''}" role="tab" aria-selected="${i === 0}" aria-label="Image ${i + 1} sur ${slides.length} : ${(s.alt || '').replace(/"/g, '&quot;')}" data-slide="${i}"></button>`).join('')
  ...
  const aboutCol1 = aboutParas
    ...
    .map((p) => `<p class="ej-about-text">${String(p).replace(/</g, '&lt;')}</p>`)
    .join('')
